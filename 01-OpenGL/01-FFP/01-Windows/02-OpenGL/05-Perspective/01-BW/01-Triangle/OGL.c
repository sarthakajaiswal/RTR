...
#include <gl/GL.h>  
#include <gl/GLU.h>  // '\' and '/' both are valid in windows but only '/' is allowed in others (linux) 
...
#pragma comment(lib, "glu32.lib") 
                        /* 
                            Note - this is not windowing utility (like freeGLUT) 
                            microsoft developed which used openGL functions and made some utilities  
                            eg - sphere. Also we are using it for perspective 
                        */ 
...
void resize(int width, int height) 
{
    ...
    /* as in orthographic OpenGL has function for creating perspective volume too - glFrustum() */
    /* as it is harder to use we use utility function here */

    /*
        to convert gluPerspective into glFrustum 
        Glfloat H = tan((fovy/2) / 180 * PI) * near; 
        GLfloat W = aspectRatio * H; 

        glFustum(-W, W, -H, H, near, far);  
    */
    
    gluPerspective(         /* glu32.dll */
        45.0f,      /* 1) fovy - field of view y -> angle between top and bottom */
        ((GLfloat)width / (GLfloat)height), /* 2) aspect ratio */
        0.1f,       /* 3) near - jawalchi bhint */
        100.0f      /* 4) far  - lambhchi bhint */
    ); 

    /* 
        glOrtho() -> gluOrtho2D() in glu32.dll 
        gluOrtho2D(left, right, bottom, top) 
        near and far are taken as -1 and 1 internally.     
    */
}
...
void display(void) 
{
    ...
    // translate triangle backward by z 
