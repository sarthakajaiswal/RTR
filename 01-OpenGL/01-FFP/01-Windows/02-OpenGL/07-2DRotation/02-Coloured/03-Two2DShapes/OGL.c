...
void display(void)
{
	...
	glMatrixMode(GL_MODELVIEW); // OpenGL chya engine/library la sangane 
								// ja modelview matrix la jaa ani saglyat vrcha matrix gheun ye 
	glLoadIdentity();
...
void uninitialize(void)
{
	...
	if (gpFile)
	{
		fprintf(gpFile, "Programm Terminated Successfully.");
		fclose(gpFile);
		gpFile = NULL;
	}
}
