...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iShowCmd) 
{
    ...
    RECT screenSize; 
    SystemParametersInfo(SPI_GETWORKAREA, sizeof(screenSize), &screenSize, 0); 
    float width = screenSize.right - screenSize.left; 
    float height = screenSize.bottom - screenSize.top; 

    float winWidth = (width/2) - (WIN_WIDTH/2); 
    float winHeight = (height/2) - (WIN_HEIGHT/2); 
